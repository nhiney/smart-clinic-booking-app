...
## 📱 4. Chạy các Module Thiết bị riêng biệt (Multi-Device)

Dự án hỗ trợ chạy độc lập từng module dành riêng cho các thiết bị khác nhau tại phòng khám:

### 🔹 Thiết bị Người dùng (User Device)
Dành cho bệnh nhân cài đặt trên điện thoại để đặt lịch và xem hồ sơ.
```bash
flutter run -t lib/main_user_device.dart
```

### 🔹 Thiết bị Quét mã (QR Scanner Device)
Dành cho máy tính bảng đặt tại quầy tiếp đón của phòng khám (Khuyên dùng chế độ xoay ngang).
```bash
...
## 🛠️ 5. Cấu hình Firebase (Nếu chuyển dự án mới)
